...
%% plot results
x_axis = (1:time_step_number)*sample_second/60/60;
for ii = 1:precast_beam_number-1  %correlation coefficients of every time-step
    figure
    scatter(x_axis,correlation_coefficient(ii,:),10,'filled');
    xlabel('Time(h)')
    ylabel('Correlation Coefficient')
    legend(['\rho_',num2str(ii),'_',num2str(ii+1)])
    set(gca,'FontSize',13)
    xlim([0,simulation_hour])
    xticks(0:simulation_hour/6:simulation_hour)
end

figure  %plot index
plot(x_axis, correlation_coefficient_mean(1,:),'Marker','<',...
    'MarkerIndices',time_step_number/10:time_step_number/10:time_step_number,...
    'MarkerSize',7,'LineWidth',1.5);hold on
plot(x_axis, correlation_coefficient_mean(2,:),'Marker','s',...
    'MarkerIndices',time_step_number/10:time_step_number/10:time_step_number,...
    'MarkerSize',7,'LineWidth',1.5);hold on
plot(x_axis, correlation_coefficient_mean(3,:),'Marker','x',...
    'MarkerIndices',time_step_number/10:time_step_number/10:time_step_number,...
    'MarkerSize',7,'LineWidth',1.5);hold on
plot(x_axis, correlation_coefficient_mean(4,:),'Marker','>',...
    'MarkerIndices',time_step_number/10:time_step_number/10:time_step_number,...
    'MarkerSize',7,'LineWidth',1.5);
legend('DSSM_{12}','DSSM_{23}','DSSM_{34}','DSSM_{45}')
xlabel('Time(h)')
ylabel('Index')
xlim([0,simulation_hour])
xticks(0:simulation_hour/6:simulation_hour)