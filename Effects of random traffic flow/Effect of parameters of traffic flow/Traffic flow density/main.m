...
%%
x_axis = [0.5,1:8,10:2:20,25:5:40];
index = zeros(precast_beam_number-1, length(x_axis));
counter = 0;
for average_interval_of_generating_vehicle = x_axis
    counter = counter + 1;
...
%% plot results
figure
plot(x_axis,index(1,:),'Marker','<','MarkerIndices',4:3:19,'MarkerSize',7,'LineWidth',1.5);hold on
plot(x_axis,index(2,:),'marker','s','MarkerIndices',4:3:19,'MarkerSize',7,'linewidth',1.5);hold on
plot(x_axis,index(3,:),'marker','x','MarkerIndices',4:3:19,'MarkerSize',7,'linewidth',1.5);hold on
plot(x_axis,index(4,:),'marker','>','MarkerIndices',4:3:19,'MarkerSize',7,'linewidth',1.5);
xlim([x_axis(1),x_axis(end)])
xticks([0.5,2:8,10:2:20,25:5:40])
legend('DSSM_{12}','DSSM_{23}','DSSM_{34}','DSSM_{45}','Location','southeast')
xlabel('average interval (s)')
ylabel('Index')