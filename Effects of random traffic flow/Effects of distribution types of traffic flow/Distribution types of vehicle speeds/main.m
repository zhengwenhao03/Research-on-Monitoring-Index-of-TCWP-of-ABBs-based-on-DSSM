...
distribution_type = ["normal","gamma","Weibull"];
index = zeros(precast_beam_number-1,length(distribution_type));
counter = 0;
for iType = distribution_type
    counter = counter + 1;
    [traffic] = get_vehicle_speed(traffic,vehicle_speed_mean,vehicle_speed_std,iType);
...
bar(xlabel,index)
legend('normal distribution','gamma distribution','Weibull distribution')
ylim([0.92,0.96])
ylabel('index')