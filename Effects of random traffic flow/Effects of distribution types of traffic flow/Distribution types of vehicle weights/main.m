...
[traffic] = get_vehicle_speed(traffic,vehicle_speed_mean,vehicle_speed_std,"default");

distribution_type = ["normal", "lognormal", "exponential", "gamma", "Weibull", "extreme value"];
index = zeros(precast_beam_number-1,length(distribution_type));
counter = 0;
for iType = distribution_type
    counter = counter + 1;
    [traffic] = get_vehicle_weight(traffic,...
        ...
        probability_generating_middle_vehicle,iType);
...
%% plot results
xlabel = categorical({'DSSM_{12}','DSSM_{23}','DSSM_{34}','DSSM_{45}'});
xlabel = reordercats(xlabel,{'DSSM_{12}','DSSM_{23}','DSSM_{34}','DSSM_{45}'});
bar(xlabel,index)
legend('normal distribution','lognormal distribution','exponential distribution',...
    'gamma distribution','Weibull distribution','extreme value distribution',...
    'Location','southeast')
ylim([0.92,0.95])
ylabel('index')