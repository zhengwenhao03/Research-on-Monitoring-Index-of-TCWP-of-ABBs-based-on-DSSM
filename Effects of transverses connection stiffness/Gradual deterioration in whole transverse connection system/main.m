...
% spring_stiffness_matrix = 2000e3*ones(precast_beam_number-1,node_number-2);
...
for k = x_axis
    counter = counter + 1;
    spring_stiffness_matrix = k*1000*ones(precast_beam_number-1,node_number-2);
    [K] = get_stiffness_matrix(precast_beam_number, node_number, elasticity_modulus, ...
        moment_of_inertia_precast_beam, bridge_length, spring_stiffness_matrix);
    [C] = get_damping_matrix(M, K);
...
%% plot results
figure
plot(x_axis,index(1,:),'Marker','<','MarkerIndices',7:7:49,'MarkerSize',7,'LineWidth',1.5);hold on
plot(x_axis,index(2,:),'marker','s','MarkerIndices',7:7:49,'MarkerSize',7,'linewidth',1.5);hold on
plot(x_axis,index(3,:),'marker','x','MarkerIndices',7:7:49,'MarkerSize',7,'linewidth',1.5);hold on
plot(x_axis,index(4,:),'marker','>','MarkerIndices',7:7:49,'MarkerSize',7,'linewidth',1.5);
xlim([x_axis(1),x_axis(end)])
xticks(0:4000:x_axis(end))
